function MultiAgentCVXRevisedv2_2()
%
    global f;
    global v;
    global vmin;
    ...
    dmin=5;
    FOVD=180;
    filename = 'merge_test_case5.mp4';
    v = VideoWriter(filename,'MPEG-4');
    open(v);
%     cars = scenario4();
%     cars = scenario5(); 5-star
    cars = scenario6(); %perpendicular
%   cars = scenario3();
%   cars = scenario2();
%   cars = intializeCars1();
%   cars = intializeCars2();
    ...
    for i=ids_s
        ...
        for j1 = length(t1)
            ...
            if abs(sqrt((x0-cars{i}.x(end))^2+(y0-cars{i}.y(end))^2))<cars{i}.r
                ...
                for k1=1:length(t1(t1~=j)) %check if this car is not in collision with other cars in the same region 
                    k2 = t1(t1~=j);
                    k=k2(k1);
                ...
                if flag==0
                    frontier_i(end+1,:)=[i j];
                end
            end
           
        end
        if length(frontier_i(:,1)==i)==0
            frontier_i(end+1,:)=[i i];
        end
    end
        for k=frontier_i(:,2)
            if cars{k}.s1==0
                getPositionUpdateInPrevScale(k)
            else
                getPositionUpdate(k)
            end
        end
...
function getPositionUpdateInPrevScale(k)
    global cars;
    [~, ~, xt_dot1, yt_dot1, ~, ~, ~]=getvelacc(cars{k}.t0, cars{k}.tsim, cars{k}.tf,  cars{k}.xo, cars{k}.x1, cars{k}.x2, cars{k}.x3, cars{k}.x4, cars{k}.x5, cars{k}.xf, cars{k}.yo,  cars{k}.ko, cars{k}.k1_cont, cars{k}.k2_cont, cars{k}.k3_cont, cars{k}.k4_cont, cars{k}.kf);
    cars{k}.x(end+1) = cars{k}.x(end)+cars{k}.sprev*xt_dot1*0.1; %*0.1;
    cars{k}.y(end+1) = cars{k}.y(end)+cars{k}.sprev*yt_dot1*0.1; %*0.1;
    cars{k}.vcurr = [cars{k}.sprev*xt_dot1, cars{k}.sprev*yt_dot1];
    tau_range = (1/cars{k}.sprev)*(cars{k}.tf-cars{k}.tsim) ;%*(0.1);
    incrp = (0.1*(cars{k}.tf-cars{k}.tsim))/(tau_range);
    if cars{k}.tsim<cars{k}.tf
        cars{k}.tsim = cars{k}.tsim+incrp; 
    else
        cars{k}.tsim = cars{k}.tf;
    end
    cars{k}.sprev1(end+1) = cars{k}.sprev;
    plotCars();
       
end

function[d_ids] = detectDeadLocks()
    global cars; 
    d_ids=[];
    for i1=1:length(cars)
        if length(d_ids)>0
           if ismember (i1, d_ids(:,2))
               continue;
           else
               i = i1;
           end
        ...
        if (cars{i}.s1==0 || cars{i}.s1<=0.01)
           k=1:length(cars);
           for j=k(k~=i)
                if abs(sqrt((cars{i}.x(end)-cars{j}.x(end))^2+(cars{i}.y(end)-cars{j}.y(end))^2))<5
                    d_ids(end+1,:)=[i, j];
                end
           end
        end
...
function [s1]= calculateScale(j, ids, a, b, c,xdot,ydot, xdotc, ydotc, xddotc, yddotc) 
    ...
    xpref = (xdot^2+ydot^2)/(xdotc^2+ydotc^2) ;
    ...
    while (sprev(end) - s1(1))^2>=0.01
        ...
        [s1,fval,exitflag] = fmincon(cost,[0;1],A,real(b1),Aeq,beq,[0 1],[4 1],nonlincon(),opts);
...
%         rAB1 = (cars{k}.x(end)-cars{id}.x(end))*(cars{k}.x(end)) +(cars{k}.y(end)-cars{id}.y(end))*(cars{k}.y(end)); 
        %vector pointing towards the obstacle, v the direction of robot
        %velocity heading 
...
function cars= scenario6()
x0=2.5;y0=-8;xf=3;yf=5;  %The robot points
xw1=(x0+xf)/2; 
yw1=(y0+yf)/2; 
t0 = 0; tf = 13;
[xt_robo1,yt_robo1,xt_dot1,yt_dot1,xo_r1,x1_r1,x2_r1,x3_r1,x4_r1,x5_r1,xf_r1,yo_r1,ko_r1,k1_cont_r1,k2_cont_r1,k3_cont_r1,k4_cont_r1,kf_r1] = getBernstein(x0,y0,xw1,yw1,xf,yf,t0,tf);
    ...
    x20=-1.5;y20=-3;xf=10;yf=-2.5; %The obstacle points
    xw1=(x20+xf)/2; 
    yw1=(y20+yf)/2; 
    t0=0; tf=13;
    ...
    car2.lookahead=[];
    car2.incr=0.1;
    car2.tend=0; %at what time does look ahead expire? 
    car2.sprev1=[1 1];
    car2.vcurr = [0 0];
    cars{2}=car2;
...
function cars = scenario5()
    t0=0;tf=18;    
    x0=-10;y0=10;xf=40;yf=0;
   ...
   [xt_robo1,yt_robo1,xt_dot1,yt_dot1,xo_r1,x1_r1,x2_r1,x3_r1,x4_r1,x5_r1,xf_r1,yo_r1,ko_r1,k1_cont_r1,k2_cont_r1,k3_cont_r1,k4_cont_r1,kf_r1] = getBernstein(x0,y0,xw1,yw1,xf,yf,t0,tf);
    ...
    %robot 4 
    t0=0;tf=20;
    x40=-5;y40=25;xf=43;yf=-10;
    xw1= 20;% (x40+xf)/2; 
    yw1= 15; %(y40+yf)/2; 
    [xt_robo4, yt_robo4, xt_dot4, yt_dot4, xo_r4, x1_r4, x2_r4, x3_r4, x4_r4,x5_r4, xf_r4, yo_r4,  ko_r4, k1_cont_r4, k2_cont_r4, k3_cont_r4, k4_cont_r4, kf_r4] = getBernstein(x40,y40,xw1,yw1,xf,yf,t0,tf);
    ...
    t0=0;tf=20;
    x50=-10;y50=20;xf=30;yf=-10;
    xw1=20;% (x50+xf)/2; 
    yw1=15;% (y50+yf)/2; 
    [xt_robo5, yt_robo5, xt_dot5, yt_dot5, xo_r5, x1_r5, x2_r5, x3_r5, x4_r5,x5_r5, xf_r5, yo_r5,  ko_r5, k1_cont_r5, k2_cont_r5, k3_cont_r5, k4_cont_r5, kf_r5] = getBernstein(x50,y50,xw1,yw1,xf,yf,t0,tf);
    car5 = struct; 
    car5.t0=t0;
    car5.tf=tf;
...
%     cars{2} = car2;
    ...
    t0=0;tf=19;
    x30=4.9;y30=25;xw1=10;yw1=15;xf=43;yf=0;
    [xt_robo3, yt_robo3, xt_dot3, yt_dot3, xo_r3, x1_r3, x2_r3, x3_r3, x4_r3, x5_r3, xf_r3, yo_r3,  ko_r3, k1_cont_r3, k2_cont_r3, k3_cont_r3, k4_cont_r3, kf_r3] =getBernstein(x30,y30,xw1,yw1,xf,yf,t0,tf);
    ...
    t0=0;tf=22;
    x40=7;y40=18.31;xw1=20;yw1=3;xf=40;yf=0;
    [xt_robo4, yt_robo4, xt_dot4, yt_dot4, xo_r4, x1_r4, x2_r4, x3_r4, x4_r4, x5_r4, xf_r4, yo_r4,  ko_r4, k1_cont_r4, k2_cont_r4, k3_cont_r4, k4_cont_r4, kf_r4] =getBernstein(x40,y40,xw1,yw1,xf,yf,t0,tf);
